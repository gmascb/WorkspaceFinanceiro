...
public class FinLAN extends JInternalFrame {
	...
	public FinLAN() throws PropertyVetoException {
		...
		tblLan.setModel(new DefaultTableModel(new Object[][] {}, new String[] {
				...
				"Pagar ou Receber", "Status", "Data Baixa", "Valor Baixado" }));
		...
		// TODO BOTAO DE CANCELAR A EDICAO
	...
	public String getMaxIdLan() {
		...
		try {
			...
			// pega o novo IDLAN
			ResultSet novoIdlan = acessaBanco
					.aplicaQueryComRetorno("SELECT MAX(IDLAN)+1 AS 'MAXIDLAN' FROM FLAN");
			novoIdlan.next();
